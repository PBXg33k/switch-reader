...
FC_Rect FC_DrawBoxColor(FC_Font* font, FC_Target* dest, FC_Rect box, SDL_Color color, FC_AlignEnum align, const char* formatted_text, ...)
{
    Uint8 useClip;
    if(formatted_text == NULL || font == NULL)
        return FC_MakeRect(box.x, box.y, 0, 0);
...
Uint16 FC_GetHeight(FC_Font* font, const char* formatted_text, ...)
{
    ...
    return font->height*numLines + font->descent*numLines + font->lineSpacing*(numLines - 1);  //height*numLines;
}
